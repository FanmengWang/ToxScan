...
class ToxScanModel(BaseUnicoreModel):
    ...
    def __init__(self, args, dictionary):
        ...
        self.RT_classification_heads = MyClassificationHead(
        ...
        self.EYE_classification_heads = MyClassificationHead(
            input_dim=self.args.encoder_embed_dim,
            inner_dim=self.args.encoder_embed_dim,
            num_classes=2,
            activation_fn=self.args.pooler_activation_fn,
        ...
        self.CT_classification_heads = MyClassificationHead(
            input_dim=self.args.encoder_embed_dim,
            inner_dim=self.args.encoder_embed_dim,
            num_classes=4,
            activation_fn=self.args.pooler_activation_fn,
        ...
        self.CYP_classification_heads = MyClassificationHead(
            input_dim=self.args.encoder_embed_dim,
            inner_dim=self.args.encoder_embed_dim,
            num_classes=5,
            activation_fn=self.args.pooler_activation_fn,
        ...
        self.NSR_classification_heads = MyClassificationHead(
            input_dim=self.args.encoder_embed_dim,
            inner_dim=self.args.encoder_embed_dim,
            num_classes=12,
            activation_fn=self.args.pooler_activation_fn,
        ...
        self.regression_heads = MyClassificationHead(
            input_dim=self.args.encoder_embed_dim,
            inner_dim=self.args.encoder_embed_dim,
            num_classes=self.regression_num,
            activation_fn=self.args.pooler_activation_fn,
            pooler_dropout=self.args.pooler_dropout,
        )
    ...
    def forward(
        ...
        **kwargs
        ...
        def get_dist_features(dist, et, bf0, bf1, bf2):
            n_node = dist.size(-1)
            gbf_feature = self.gbf(dist, et)
            gbf_result = self.gbf_proj(gbf_feature)
            graph_attn_bias = gbf_result + bf0 + bf1 + bf2
        ...
        graph_attn_bias = get_dist_features(src_distance, src_edge_type, bond_feature0, bond_feature1, bond_feature2)
        ...
        classification_logit = torch.cat(
            [self.classification_heads[classification_head_name](encoder_rep),
             self.RT_classification_heads(encoder_rep),
             self.EYE_classification_heads(encoder_rep),
             self.CT_classification_heads(encoder_rep),
             self.CYP_classification_heads(encoder_rep),
             self.NSR_classification_heads(encoder_rep)
             ],
            dim=-1)
        
        regression_logit = self.regression_heads(encoder_rep)
