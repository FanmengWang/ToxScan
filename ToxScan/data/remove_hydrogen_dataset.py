...
class RemoveHydrogenDataset(BaseWrapperDataset):
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        if self.remove_hydrogen:
            ...
            atom_feature = atom_feature[mask_hydrogen]
            bond_feature = bond_feature[mask_hydrogen][:,mask_hydrogen]
        ...
        if not self.remove_hydrogen and self.remove_polar_hydrogen:
            ...
            if end_idx != 0:
                ...
                atom_feature = atom_feature[:-end_idx]
                bond_feature = bond_feature[:-end_idx][:,:-end_idx]
                
        dd[self.atoms] = atoms
        dd[self.coordinates] = coordinates.astype(np.float32)
        dd[self.atom_feature] = atom_feature.astype(np.int)
        dd[self.bond_feature] = bond_feature.astype(np.int)
        return dd
