...
class TTADataset(BaseWrapperDataset):
    def __init__(self, dataset, seed, atoms, coordinates, atom_feature, bond_feature, fingerprint, target, classification_target, regression_target, conf_size=10):
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        atom_feature = self.dataset[smi_idx][self.atom_feature]
        bond_feature = self.dataset[smi_idx][self.bond_feature]
        fingerprint = self.dataset[smi_idx][self.fingerprint]
        
        smi = self.dataset[smi_idx]["data"]
        target = self.dataset[smi_idx]["target"]
        classification_target = self.dataset[smi_idx]["classification_target"]
        regression_target = self.dataset[smi_idx]["regression_target"]
