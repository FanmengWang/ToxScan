...
class CroppingDataset(BaseWrapperDataset):
    def __init__(self, dataset, seed, atoms, coordinates, atom_feature, bond_feature, max_atoms=256):
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        atom_feature = dd[self.atom_feature]
        bond_feature = dd[self.bond_feature]
        if self.max_atoms and len(atoms) > self.max_atoms:
            with data_utils.numpy_seed(self.seed, epoch, index):
                ...
                atom_feature = atom_feature[index]
                bond_feature = bond_feature[index][:, index]
        dd[self.atoms] = atoms
        dd[self.coordinates] = coordinates.astype(np.float32)
        dd[self.atom_feature + '0'] = atom_feature.astype(np.int)[:, 0]
        dd[self.atom_feature + '1'] = atom_feature.astype(np.int)[:, 1]
        dd[self.bond_feature + '0'] = bond_feature.astype(np.int)[:, :, 0]
        dd[self.bond_feature + '1'] = bond_feature.astype(np.int)[:, :, 1]
        dd[self.bond_feature + '2'] = bond_feature.astype(np.int)[:, :, 2]
        return dd
